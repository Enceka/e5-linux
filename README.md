...
| Bluetooth | ✅ **verified on the device** — `hci0` comes up on its own (`e5-bt-attach.service` holds `/dev/ttyBT0`), bluez reports `Powered: yes`, an inquiry finds devices; the BD address is the chip's default rather than the factory MAC -- docs/FINDINGS.md section 8.7 |
